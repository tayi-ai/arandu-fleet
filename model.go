...
// Fleet is the entity this package owns.
//
// It embeds the model, so a row returned by a query carries the connection and
// can be saved again. Build new rows through Fleets: a struct literal has no
...
// Fleets returns the configured model for the fleets table.
//
...
// Resource is the list of fields one Fleet is allowed to answer with.
//
...
func NewResource(record Fleet) Resource {
	return newResource(record.ID, record.Name, record.CreatedAt)
}

func resourceFromPointer(record *Fleet) Resource {
	if record == nil {
		return Resource{}
	}
	return newResource(record.ID, record.Name, record.CreatedAt)
...
func NewCollection(records []Fleet, cursor string) Collection {
...
func collectionFromPointers(records []*Fleet, cursor string) Collection {
	resources := make([]Resource, 0, len(records))
	for _, record := range records {
		if record != nil {
